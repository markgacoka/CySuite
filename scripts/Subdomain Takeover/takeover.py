...
def massTakeOver(targets, threads):
	...
	if len(targets) <= 100000:
		print("{0} {1} {2}".format("\n[*]-Scanning", len(targets), "domains for potential takeover..."))
	else:
		print("{0} {1} {2}".format("\n[*]-Scanning", len(targets), "domains for potential takeover, in chunks of 100,000..."))

	targetChunks = list(chunks(list(targets), 100000))
	iteration = 1
	...
	for targetChunk in targetChunks:
		with ThreadPoolExecutor(max_workers=threads) as executor:
			...
			try:
				...
				completed = tqdm(completed, total=len(targetChunk), desc="  \__ {0}".format("Progress"), dynamic_ncols=True, leave=leaveFlag)
...
def init(resolved_hosts, threads=100):
	...
	print("{0} {1}".format("Takeover vulnerabilities that were identified:", len(results_json)))

	for key, values in list(results_json.items()):
		print(key, ":", ", ".join(str(value) for value in values))
